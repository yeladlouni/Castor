...
class SemevalTrainer(Trainer):
    def train_epoch(self, epoch):
        ...
        if self.use_tensorboard:
            self.writer.add_scalar('semeval/train/kl_div_loss', total_loss / len(self.train_loader.dataset.examples), epoch)
    ...
    def train(self, epochs):
        ...
        for epoch in range(1, epochs + 1):
            ...
            if self.use_tensorboard:
                self.writer.add_scalar('semeval/lr', self.optimizer.param_groups[0]['lr'], epoch)
                self.writer.add_scalar('semeval/dev/accuracy', accuracy, epoch)
                self.writer.add_scalar('semeval/dev/kl_div_loss', new_loss, epoch)
